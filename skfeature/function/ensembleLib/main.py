...
from skfeature.function.ensembleLib import listOfClusterPr,clusterEnsemble,evalMutual,ClusterEnsembleTest,joint_criterion,JointTest,Cluster_select,ClusterSelectTest,Random_select
from skfeature.function.ensembleLib.SpectralJordan import SpectralJordan
...
def main():
    ...
    normalize_status=2
    c=8
    X = scipy.io.loadmat('C:\MATLAB\mfilesICML\EnsembleClustering\Classification\yeast.mat')
    X = X['yeast']  # data
    X = X.astype(float)
    rowLen = len(X)
    colLen = len(X[0])
