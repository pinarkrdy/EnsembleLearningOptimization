...
sq =SqlLiteDatabase.SqlLiteDatabase('C:\Users\lenovo\Desktop\scikit-feature-master\skfeature\FeatureSelectionDB.db')
...
resultTrain=[]
for s in range(4):
    resultTrain.append(sq.get('XTrain' , 'selected'))
    trainTemp=str(resultTrain).split("_")[2].split("',)")[0]
    train_list=np.asarray(trainTemp.split(",")).astype(int)
    trainResult=X[train_list,:]
    trainY=y[train_list]

    resultTest.append(sq.get('XTest' , 'selected'))
    testTemp=str(resultTest).split("_")[2].split("',)")[0]
    test_list=np.asarray(testTemp.split(",")).astype(int)
    testResult=X[test_list,:]
    testY=y[test_list]

    resultVal.append(sq.get('XVal', 'selected'))
    valTemp = str(resultVal).split("_")[2].split("',)")[0]
    val_list =np.asarray(valTemp.split(",")).astype(int)
    ...
    resultMatrix = resultMatrix[:, 1:resultMatrix.shape[1]]
    ...
    for i in range(rowLen):
        for j in range(rowLen):
            if i == j:
                ...
                X_train1, X_test1, y_train1, y_test1 = train_test_split(prunedX, trainY, test_size=0.2)
                # svm classification
    ...
    for index in range(len_res):
        ...
        X_train2, X_test2, y_train2, y_test2 = train_test_split(prunedX2, valY, test_size=0.2)
        # svm classification
